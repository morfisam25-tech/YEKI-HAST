...
`;
  ...
  try {
    ...
    for (const line of e2e.stdout.split(/\r?\n/)) {
      if (/^(PRODUCTION_HARD_BLOCK|PRODUCT_FLOW|REAL_PEER_CONNECTION|SELECTED_ICE|CALLER_TO_LISTENER_AUDIO_RTP|LISTENER_TO_CALLER_AUDIO_RTP|PRODUCT_CALL_LIFECYCLE|BILLING_SIMULATION|SETTLEMENT_IDEMPOTENCY|W10_PRODUCT_AUDIO_E2E_AUTOMATED_READY)=?[A-Z_]*$/.test(line)) {
