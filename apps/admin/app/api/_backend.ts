...
export function backendBaseUrl(): string {
  ...
  if (env === 'preview_internal_beta') {
    ...
    if (!configured) {
      throw new Error('ADMIN_API_BASE_URL is required in preview_internal_beta; Preview must never fall back to the Production API');
    }
    const normalized = configured.replace(/\/$/, '');
    if (isProductionOrigin(normalized, PRODUCTION_API_BASE_URL)) {
      throw new Error('ADMIN_API_BASE_URL must not point at the Production API origin in preview_internal_beta');
    }
    return normalized;
  }
