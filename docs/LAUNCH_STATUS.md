...
- Production runtime is pinned to Node 22, matching CI/build validation.
...
- Admin production app: deployed; protected operations still require a real admin session.
...
- Caller voice launch: production telephony contract + verified adapter semantics.
...
Once the external credentials exist securely:

1. Rerun Foundation QA on current `main` and require a true green run.
2. Add only the required repository secrets through secure GitHub UI/storage.
3. Trigger the controlled Production API workflow.
4. Require DB verification without migrations.
5. Require `/health` 200, `/ready` 200 and a valid `/v1/bootstrap` response.
6. Require real Email OTP delivery, verification, authenticated session and logout/revocation E2E PASS.
7. Deploy Web and Admin through the exact UNIQUE frontend workflow and require both live smoke gates.
8. Review Admin integration-readiness output with secrets excluded.
9. Keep Caller beta closed until telephony/payment/phone/age gates are all genuinely ready.
10. Only then enable the smallest intended beta surface and monitor runtime errors.
