...
export async function handleApiRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    // TEMPORARY W88 PURGE PROOF -- REMOVE BEFORE FINAL W88 TREE (begin)
    if (method === 'POST' && url.pathname === '/v1/internal-beta/owner-test/recording-archive-purge-proof') { const { requireInternalOwnerTestMode } = await import('./lib/internal-owner-test.ts'); requireInternalOwnerTestMode(); ensureDatabaseReady(); const { runRecordingArchivePurgeProof } = await import('./routes/internal-owner-test-purge-proof.ts'); return await runRecordingArchivePurgeProof(req, res); }
    // TEMPORARY W88 PURGE PROOF -- REMOVE BEFORE FINAL W88 TREE (end)
