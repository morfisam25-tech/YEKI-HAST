...
export async function handleApiRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    if (method === 'POST' && url.pathname === '/v1/admin/wallet-credits') { ensureDatabaseReady(); const { createAdminWalletCredit } = await import('./routes/admin-payments.ts'); return await createAdminWalletCredit(req, res); }
