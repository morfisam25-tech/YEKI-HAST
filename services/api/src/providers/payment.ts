...
// Zibal trackId is a provider-assigned positive integer, returned as a JSON
// number; bounded to a JS-safe integer so it round-trips exactly through
// Number(...) on the verify call (see ZibalProvider#verifyPayment).
...
class ZibalProvider implements PaymentProvider {
  ...
  constructor() {
    ...
    // Zibal's own documented "zibal" merchant id is a public sandbox/test
    // identity, not a real merchant contract -- never allow it in production
    // (mirrors the fail-closed intent NextPayProvider's UUID_RE check gives
    // NextPay's api key).
  ...
  async createPayment(input: CreatePaymentInput): Promise<CreatePaymentResult> {
    ...
    // Zibal's request/verify amount fields are Rials (IRR minor units), the
    // same unit app.payment_attempts.amount_minor already stores -- no
    // Toman conversion (x10/÷10) belongs anywhere on this path.
  ...
  async verifyPayment(input: VerifyPaymentInput): Promise<VerifyPaymentResult> {
    ...
    return {
      paid,
      // result=100 without the successful status stays pending in the route
      // (never a bare "100 means paid" read) and can never credit a wallet.
  ...
  async refund(_providerPaymentId: string, _amountMinor: bigint, _currencyCode: 'IRR' | 'IRT'): Promise<void> {
    // Zibal refunds require separately activated corporate banking that is
    // not part of this task's approved scope. Fail closed rather than
